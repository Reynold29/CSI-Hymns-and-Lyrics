...
class Category2Screen extends StatefulWidget {
  ...
  @override
  _Category2ScreenState createState() => _Category2ScreenState();
}
...
class _Category2ScreenState extends State<Category2Screen> {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController( 
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text("Jesus' Passion and Death"),
          bottom: const TabBar(
            tabs: [
              Tab(text: 'Hymns'), 
              Tab(text: 'Keerthane'), 
              ...
              FutureBuilder<List<Hymn>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    final filteredHymns = hymns.where((hymn) => hymn.number >= 91 && hymn.number <= 107).toList();
              ...
              FutureBuilder<List<Keerthane>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    final filteredKeerthane = keerthaneList.where((keerthane) => keerthane.number >= 64 && keerthane.number <= 73).toList();
                    return ListView.builder(
